...
from src.backend.llm import get_llm_response
from src.backend.utils.std_utils import reader, json_reader, json_writer, writer, append_writer
import numpy as np
...
class HITLllm:
    def __init__(self, human_nl_demo, state_path, preprompts_dirpath="config/llm_preprompts", is_first_demo=False):
...
if __name__ == "__main__":
    all_yes = False
    NL_FEEDBACK_PATH = "test/demonstrations/nl_feedback.txt"
    with open(NL_FEEDBACK_PATH, "r") as f:
        nl_feedbacks = f.readlines()
    examples_nums = np.arange(1, 4)  # ordered
    ...
    for i in examples_nums:
        ...
        lcl_sketches_path = "test/demonstrations/seqn_lfps_sketches.txt"
        if is_first_demo:
