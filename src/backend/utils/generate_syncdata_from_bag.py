...
def save_pointcloud_to_bin(pointcloud_msg, filename, lcc: JackalLidarCamCalibration, pointclouds_out_dir, raw=False):
...
if __name__ == "__main__":
    for bagnumber in range(1, 4):
        print(f"Generating data for bag {bagnumber}.bag")
        bag_path = f"test/demonstrations/{bagnumber}.bag"
        out_dir = f"test/demonstrations/syncdata/{bagnumber}"
        ...
        generate3(bag_path, out_dir, IMAGE_TOPIC, POINTCLOUD_TOPIC, LOCALIZATION_TOPIC,
                  syncimg_pc_min_lag=0.015, syncimg_pc_max_lag=0.055, syncimg_abs_loc_threshold=0.02, user=True, filename_prefix=prefix)
