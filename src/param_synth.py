...
from src.backend.utils.std_utils import json_writer, json_reader
from src.datagen import LDIPSdatagenSingleEx
import numpy as np
...
def LDIPS_synthesize(examples_data, params_sketch_lsps, pid=0):
    ...
    # Write to file
    TEMP_ROOTDIR = f"test/demonstrations/param_synth/_ldips_temp_{pid}"
    ...
    args = ("third_party/pips/bin/srtr",
...
def read_data(nums_list):
    data_table_dir = "test/demonstrations/SSR"
...
if __name__ == "__main__":
    ...
    seqn_lfps_sketches = json_reader("test/demonstrations/seqn_lfps_sketches.json")
    for i in range(1, 4):
        print(f"Processing {i} of 3")
    ...
    json_writer("test/demonstrations/seqn_filled_lfps_sketches.json", seqn_filled_lfps_sketches)
